...
<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <a href="{{ route('admin.quran.exams.index', ['year' => substr($month, 0, 4)]) }}" class="text-sm text-emerald-700 hover:text-emerald-800">← العودة لشبكة الأشهر</a>
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.quran.exams.month', $previousMonth) }}" class="bg-white border border-gray-300 text-gray-700 text-sm font-bold px-3 py-2 rounded-lg">الشهر السابق ←</a>
            <form onsubmit="return false;">
                <input type="month" value="{{ $month }}" onchange="window.location.href='{{ url('admin/quran/exams/month') }}/' + this.value"
                       class="border border-gray-300 rounded-lg px-3 py-2 text-sm" aria-label="الشهر">
            </form>
            <a href="{{ route('admin.quran.exams.month', $nextMonth) }}" class="bg-white border border-gray-300 text-gray-700 text-sm font-bold px-3 py-2 rounded-lg">→ الشهر التالي</a>
        </div>
    </div>
