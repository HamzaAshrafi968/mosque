...
<div class="max-w-5xl mx-auto space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h2 class="text-2xl font-extrabold text-gray-800">{{ $session ? 'تعديل تسميع' : 'تسجيل تسميع جديد' }}</h2>
            <p class="text-sm text-gray-500 mt-1">
                {{ $student->name }} — {{ $type->label() }} — {{ $date }} — صفحة {{ $fromPage }} → صفحة {{ $toPage }}
            </p>
        </div>
        <a href="{{ $session ? route('admin.quran.tasmee.edit', $session) : route('admin.quran.tasmee.create') }}" class="text-sm text-emerald-700 hover:text-emerald-800">← رجوع للنموذج</a>
    </div>
    ...
    <form method="POST" action="{{ $session ? route('admin.quran.tasmee.update', $session) : route('admin.quran.tasmee.store') }}" data-quran-preview-form class="space-y-5">
        @csrf
        @if($session)
            @method('PATCH')
        @endif
