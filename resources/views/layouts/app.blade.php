...
<head>
    ...
    <title>@yield('title', 'مؤسسة السَّفَرَة للعلوم والتنمية') | {{ config('app.name', 'مؤسسة السَّفَرَة للعلوم والتنمية') }}</title>
...
<body class="bg-[#f4f6f4] min-h-screen font-sans antialiased">
    ...
    @if (auth()->user()->isAdmin())
        <header
            class="gradient-sidebar relative text-white shadow-[0_14px_34px_-16px_rgba(5,32,25,0.65)] sticky top-0 z-40">
            ...
            <div class="relative max-w-screen-2xl mx-auto px-3 sm:px-6 py-2.5 flex items-center gap-2.5 sm:gap-3">
                ...
                <div class="flex items-center gap-2.5 font-bold shrink-0 min-w-0">
                    <span
                        class="w-10 h-10 rounded-xl p-[1.5px] bg-gradient-to-br from-gold-200 via-gold-400 to-gold-600 shadow-lg shadow-gold-950/20 shrink-0">
                    ...
                    <div class="leading-tight min-w-0">
                        <div class="text-sm sm:text-base font-extrabold truncate">جامع
                            {{ auth()->user()->tenant?->name }}</div>
                        <div class="text-[10px] sm:text-[11px] text-gold-200/90 font-semibold truncate">لوحة إدارة
                            الجامع — اختر الدوام لعرض بياناته</div>
                    </div>
                </div>
    ...
    <div class="min-h-screen lg:flex">
        @if (!auth()->user()->isSuperAdmin() && !auth()->user()->isAdmin())
            <header
                class="lg:hidden sticky top-0 z-30 gradient-sidebar relative text-white flex items-center justify-between px-4 py-3 shadow-lg overflow-hidden">
                <div aria-hidden="true" class="topbar-sheen pointer-events-none absolute inset-0 opacity-20"></div>
                <button type="button" id="sidebar-toggle"
                    class="relative p-2 rounded-xl bg-white/10 hover:bg-white/20 active:scale-95 transition"
                    aria-label="القائمة">
                    <x-icon name="menu" class="w-6 h-6" />
                </button>
                <div class="relative flex items-center gap-2 font-bold">
                    <span class="bg-white rounded-lg p-1 grid place-items-center overflow-hidden"><img
                            src="{{ asset('images/logo-mark.png') }}" alt="شعار مؤسسة السَّفَرَة للعلوم والتنمية"
                            class="w-6 h-6 object-contain"></span>
                    <span class="text-sm">مؤسسة السَّفَرَة للعلوم والتنمية</span>
                </div>
                <form method="POST" action="{{ route('logout') }}" class="relative">
                    @csrf
                    <button type="submit"
                        class="p-2 rounded-xl bg-white/10 hover:bg-white/20 active:scale-95 transition"
        ...
        <aside id="sidebar"
            class="gradient-sidebar fixed inset-y-0 right-0 z-50 w-72 max-w-[85vw] text-white flex flex-col shrink-0 overflow-hidden transition-transform duration-300 ease-out translate-x-full lg:translate-x-0 lg:static shadow-2xl shadow-pine-950/40">
        ...
        <main class="flex-1 min-w-0 px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
            <div class="max-w-[1500px] mx-auto animate-page">
                @if (session('success'))
                    <div class="flash-toast relative overflow-hidden mb-6 flex items-center gap-3.5 rounded-2xl bg-white/95 backdrop-blur border border-emerald-200/80 px-4 py-3.5 shadow-[0_18px_40px_-18px_rgba(6,40,29,0.35)]"
                        data-flash>
                        <span
                            class="w-10 h-10 shrink-0 rounded-xl bg-gradient-to-br from-emerald-400 to-emerald-700 grid place-items-center text-white shadow-md shadow-emerald-600/30">
                            <x-icon name="check" class="w-5 h-5" />
                        </span>
                        <p class="flex-1 text-emerald-950 font-bold text-sm leading-relaxed">{{ session('success') }}
                        </p>
                        ...
                        <span class="flash-toast-bar bg-gradient-to-l from-emerald-400 via-emerald-500 to-emerald-700"
                            data-flash-bar></span>
                    </div>
                @endif

                @if ($errors->any())
                    <div class="flash-toast relative overflow-hidden mb-6 rounded-2xl bg-white/95 backdrop-blur border border-red-200/80 px-4 py-3.5 shadow-[0_18px_40px_-18px_rgba(120,20,20,0.3)]"
                        data-flash>
                        <div class="flex items-center gap-3.5">
                            <span
                                class="w-10 h-10 shrink-0 rounded-xl bg-gradient-to-br from-red-400 to-red-600 grid place-items-center text-white shadow-md shadow-red-600/30">
                        ...
                        <span class="flash-toast-bar bg-gradient-to-l from-red-400 via-red-500 to-red-600"
                            data-flash-bar></span>
                    </div>
                @endif
