...
<head>
    ...
    <title>@yield('title') | {{ config('app.name', 'مؤسسة السَّفَرَة للعلوم والتنمية') }}</title>
...
<body class="min-h-screen font-sans p-4 relative overflow-hidden">
    ...
    <div aria-hidden="true"
        class="animate-blob absolute -top-24 start-[-10%] w-[30rem] h-[30rem] rounded-full bg-gold-400/15 blur-3xl">
    </div>
    <div aria-hidden="true"
        class="animate-blob absolute bottom-[-20%] end-[-8%] w-[32rem] h-[32rem] rounded-full bg-emerald-300/15 blur-3xl"
        style="animation-delay:-8s"></div>
    <div aria-hidden="true"
        class="animate-blob absolute top-1/3 start-1/3 w-80 h-80 rounded-full bg-teal-200/10 blur-3xl"
        style="animation-delay:-4s"></div>

    {{-- هالة أعلى المركز خلف البطاقة --}}
    <div aria-hidden="true"
        class="absolute top-[16%] start-1/2 -translate-x-1/2 w-[560px] h-[300px] rounded-full bg-gold-300/10 blur-[100px] pointer-events-none">
    </div>
    ...
    <div class="w-full max-w-md mx-auto relative z-10 animate-fade-in-up">
        <div class="text-center mb-8 animate-fade-in-up" style="animation-delay:.1s">
            <div class="relative inline-flex items-center justify-center">
                ...
                <div
                    class="relative w-20 h-20 rounded-3xl p-[2px] bg-gradient-to-br from-gold-200 via-gold-400 to-gold-700 shadow-2xl shadow-gold-950/40">
            ...
            <h1 class="text-3xl sm:text-4xl font-black mt-5 text-white">
                مؤسسة <span class="gold-text">السَّفَرَة</span> للعلوم والتنمية
            </h1>
            <p class="text-sm mt-2 text-emerald-100/70 font-medium">نظام إدارة المساجد وحلقات القرآن</p>
        </div>

        <div class="glass-card rounded-[28px] shadow-[0_30px_70px_-24px_rgba(0,0,0,0.55)] border border-white/50 p-8 sm:p-10 animate-fade-in-up"
            ...
            @if ($errors->any())
                <div
                    class="flex items-start gap-3 bg-red-50 border border-red-200 text-red-800 rounded-2xl px-4 py-3.5 mb-6 text-sm animate-scale-in">
        ...
        <p class="text-center text-emerald-100/50 text-xs mt-7 animate-fade-in-up" style="animation-delay:.34s">
            جميع الحقوق محفوظة © {{ date('Y') }} — مؤسسة السَّفَرَة للعلوم والتنمية
        </p>
    </div>
</body>
