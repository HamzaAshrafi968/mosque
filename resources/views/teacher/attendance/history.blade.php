...
<div class="bg-white rounded-xl shadow overflow-hidden mb-6 p-4">
    <form method="GET" action="{{ route('teacher.attendance.history') }}" class="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
        <div>
...
@if($section)
    <div class="mb-4 flex items-center justify-between">
        <h2 class="text-lg font-bold text-gray-800">جدول حضور {{ $section->name }}</h2>
        <a href="{{ route('teacher.attendance.create', ['section_id' => $section->id]) }}"
           class="bg-emerald-700 hover:bg-emerald-800 text-white text-sm font-bold px-4 py-2 rounded-lg">تسجيل حضور جديد</a>
    </div>
    <x-attendance-history-grid :sessions="$sessions" :rows="$rows" />
@else
    <div class="bg-white rounded-xl shadow p-6 text-center text-gray-500">لا توجد شعب موكلة إليك</div>
@endif
@endsection
