...
<div class="max-w-6xl mx-auto space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h2 class="text-2xl font-extrabold text-gray-800">✅ اختبارات الحفاظ — {{ $monthLabel($month) }}</h2>
            <p class="text-sm text-gray-500 mt-1">حفاظك ضمن نطاقك وغير المسندين لمشرف آخر — «لم يُختبر» لا تعني «راسب»</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('teacher.quran.exams.index', ['month' => $previousMonth]) }}" class="bg-white border border-gray-300 text-gray-700 text-sm font-bold px-3 py-2 rounded-lg">السابق ←</a>
            <form method="GET" action="{{ route('teacher.quran.exams.index') }}">
            ...
            <a href="{{ route('teacher.quran.exams.index', ['month' => $nextMonth]) }}" class="bg-white border border-gray-300 text-gray-700 text-sm font-bold px-3 py-2 rounded-lg">→ التالي</a>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                @forelse($exams as $exam)
                    <tr class="border-t">
                        <td class="px-4 py-3 font-bold text-gray-800">{{ $exam->student->name }}</td>
                        <td class="px-4 py-3">
                            <span @class([
                        ...
                        <td class="px-4 py-3">{{ $exam->grade !== null ? $exam->grade.' / 100' : '—' }}</td>
                ...
                @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-gray-400">لا يوجد حفاظ ضمن نطاقك لهذا الشهر</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
