...
<div class="max-w-2xl mx-auto space-y-6">
    <a href="{{ route('teacher.quran.qualifying.index') }}" class="text-sm text-emerald-700 hover:text-emerald-800">← تقييماتي الأسبوعية</a>
    <h2 class="text-2xl font-extrabold text-gray-800">تسجيل تقييم أسبوعي ({{ \Carbon\Carbon::parse($weekStart)->format('Y-m-d') }} ← {{ \Carbon\Carbon::parse($weekEnd)->format('Y-m-d') }})</h2>

    <form method="POST" action="{{ route('teacher.quran.qualifying.evaluations.store') }}" class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
        @csrf
        <div>
        ...
        <div class="flex items-center justify-between pt-2">
            ...
            <a href="{{ route('teacher.quran.qualifying.index') }}" class="text-gray-500 text-sm hover:underline">إلغاء</a>
        </div>
    </form>
</div>
@endsection
