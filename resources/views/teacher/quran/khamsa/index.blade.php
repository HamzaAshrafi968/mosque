...
<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">مراجعات طلابك — كل خمسة ٥ صفحات، وكل جزء ٤ خمسات</p>
        </div>
        <a href="{{ route('teacher.quran.khamsa.create') }}" class="bg-emerald-700 hover:bg-emerald-800 text-white text-sm font-bold px-4 py-2 rounded-lg">+ تخصيص مراجعة</a>
    </div>

    <form method="GET" action="{{ route('teacher.quran.khamsa.index') }}" class="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
        <div>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('teacher.quran.khamsa.index') }}" class="bg-white border border-gray-300 text-gray-700 text-sm font-bold px-4 py-2 rounded-lg">إعادة تعيين</a>
        </div>
    </form>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 text-gray-600">
                        ...
                        <th class="px-4 py-3 text-right">الدوام</th>
                ...
                @forelse($reviews as $review)
                    ...
                    <tr class="border-t">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">{{ $review->studySession?->name ?? '—' }}</td>
                        ...
                        <td class="px-4 py-3 text-center whitespace-nowrap">
                            <a href="{{ route('teacher.quran.khamsa.show', $review) }}" class="text-xs text-emerald-700 hover:underline">عرض</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-gray-400">لا توجد مراجعات بعد — ابدأ بتخصيص مراجعة لطلابك.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
