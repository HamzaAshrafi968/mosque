...
<div class="max-w-5xl mx-auto space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <a href="{{ route('teacher.quran.ijazah.index') }}" class="text-sm text-emerald-700 hover:text-emerald-800">← برنامج الإجازة</a>
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('teacher.quran.ijazah.month', [$student, $previousMonth]) }}" class="bg-white border border-gray-300 text-gray-700 text-sm font-bold px-3 py-2 rounded-lg">الشهر السابق ←</a>
            <a href="{{ route('teacher.quran.ijazah.month', [$student, $nextMonth]) }}" class="bg-white border border-gray-300 text-gray-700 text-sm font-bold px-3 py-2 rounded-lg">→ الشهر التالي</a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                @else
                    <a href="{{ route('teacher.quran.ijazah.evaluations.create', ['student_id' => $student->id, 'month' => $month]) }}"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        @for($week = 1; $week <= 4; $week++)
            ...
            <x-ijazah-week-card
                ...
                :month="$month"
                :student="$student"
                :action="$evaluation ? route('teacher.quran.ijazah.weekly.update', $evaluation) : route('teacher.quran.ijazah.weekly.store')"
