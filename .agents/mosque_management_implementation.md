...
وبالإضافة إلى الأدوار، يستطيع مدير الجوامع تخصيص صلاحيات **مستخدم بعينه** (أستاذ/طالب/مدير جامع/ولي أمر) عبر تجاوزات مباشرة في جدول `permission_user`:

- التجاوز المباشر يتقدم على صلاحيات الدور: `effect=deny` يسحب الصلاحية، وقيمة النطاق تمنحها وتستبدل نطاق الدور.
- غياب التجاوز يعني وراثة صلاحيات الدور.
- لا يجوز منح النطاق `global` لمستخدم تابع لجامع، ويمنع مدير الجوامع نفسه من التعديل.
- المصفوفة مدمجة داخل نموذج المستخدم: `GET/PATCH /super-admin/mosques/{mosque}/users/{user}/edit|update`
  يحفظ البيانات والدور والتخصص والدوام والصلاحيات معاً (تبقى صفحة المصفوفة المنفصلة متاحة للربط المباشر).
...
## تخصصات الجداول (Programs) — مواصفة مستقلة

المواصفة الكاملة في `.agents/mosque_management_schedule_programs.md`. المُلخَّص:

```text
programs            برنامج/تخصص: التحفيظ، الإجازة، اختبارات الحفظ، الدورات الشرعية، البرامج القرآنية، مخصص
program_periods     فترات كل برنامج (الفترة الأولى/الثانية...) مع أوقات اختيارية
program_attributes  خصائص مخصصة لكل برنامج (نوع الحقل، مطلوب، خيارات، قيمة)
```

- الحصص تدعم إضافات `program_id`, `program_period_id`, `study_session_id` (الدوام)،
  و`subject_id` يصبح اختيارياً عند اختيار برنامج.
- اختيار فترة يملأ وقت الحصة تلقائياً من أوقات الفترة.
- الدوام النشط يرشّح الجداول أيضاً، ولكل برنامج فتراته الخاصة (قد تكون فترة واحدة).
- لكل دوام برامجه المتاحة (`program_study_session`): بلا تخصيص = كل البرامج،
  ومع التخصيص تُرشَّح النماذج والـ API ويرفض الخادم البرنامج غير المتاح للدوام.
- صلاحيات `programs.view/create/update/delete` بنطاق `mosque` للمدير، والمعلم يرى
  البرنامج والفترة داخل جدوله فقط.
- تُزرع البرامج الخمسة تلقائياً لكل جامع (جديد أو قائم) ويمكن إضافة برامج مخصصة.
